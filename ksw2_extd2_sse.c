...

{
    ...
    ksw_reset_extz(ez);
    {
      int totlen = qlen + tlen;
      uint8_t *bufhash = malloc(totlen);
      memcpy(bufhash, query, qlen);
      memcpy(bufhash+qlen, target, tlen);
      uint32_t hashx = __ac_X31_hash_string(bufhash);
      hashx ^= __ac_Wang_hash(qlen) + __ac_Wang_hash(tlen);
      hashx = __ac_Wang_hash(hashx);
      ez->pad8B[1]=hashx;
    }
    if (m <= 1 || qlen <= 0 || tlen <= 0) return;
    if (q2 + e2 < q + e) t = q, q = q2, q2 = t, t = e, e = e2, e2 = t; // make sure q+e no larger than q2+e2
    ...
    if (with_cigar) {
        ...
        //psize = ((qlen + tlen - 1) * n_col_ + 1) * 16 - ((uint8_t*)p - mem2);
        psize = ((qlen + tlen - 1) * n_col_ + 1);
    }
    //traverse query seq
    for (t = 0; t < qlen; ++t) qr[t] = query[qlen - 1 - t];//
    memcpy(sf, target, tlen); //
    ...
    for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
...
#if 0
            {
                extern int swdumpseq;
                char pname[64] = {0};
                char oname[64] = {0};
                snprintf(pname, sizeof(pname)-1, "ppp%d.txt", swdumpseq);
                snprintf(oname, sizeof(oname)-1, "off%d.txt", swdumpseq);
                FILE *fd = fopen(pname, "a");
                fprintf(fd, "pppppppppppp\n\n");
                fprintf(fd,"\nr=%d st=%d en=%d xt1=%d vt1=%d x2t1=%d x1=%d v1=%d ut=%d x21=%d\n",r,st,en,xt1,vt1,x2t1,x1,v1,ut,x21);
                for (t = st; t <= en; ++t){
                    if (t != st && (t-st)%16 == 0) fprintf(fd,"\n");
                    fprintf(fd,"s[t]=%d v[t]=%d u[t]=%d x[t]=%d y[t]=%d x2[t]=%d y2[t]=%d pr[t]=%d ",s[t],v[t],u[t],x[t],y[t],x2[t],y2[t],pr[t]);
                }
                fprintf(fd,"\n");
                fclose(fd);
        ...
        if(!approx_max){// update ez
...
#if 0
            {
                ...
                char pname[64] = {0};
                char oname[64] = {0};
                snprintf(pname, sizeof(pname)-1, "HUV%d.txt", swdumpseq);
                snprintf(oname, sizeof(oname)-1, "off%d.txt", swdumpseq);
                FILE *fd = fopen(pname, "a");
                fprintf(fd,"\nr=%d st=%d en=%d\n",r,st,en);
                if(!approx_max){
                    fprintf(fd,"Hen1=%d max_H=%d max_t=%d u[en]=%d v[en]=%d H[en]=%d ez->mte=%d ez->mte_q=%d ez->mqe=%d ez->mqe_t=%d ez->zdropped=%d ez->score=%d\n",Hen1,max_H,max_t,u[en],v[en],H[en],ez->mte,ez->mte_q,ez->mqe,ez->mqe_t,ez->zdropped,ez->score);
                }else{
                    fprintf(fd,"last_H0_t=%d v[last_H0_t]=%d u[last_H0_t + 1]=%d H0=%d ez->zdropped=%d ez->score=%d\n",last_H0_t,v[last_H0_t],u[last_H0_t + 1],H0,ez->zdropped,ez->score);
                }
                fprintf(fd,"ez->max=%d ez->max_t=%d ez->max_q=%d\n",ez->max,ez->max_t,ez->max_q);
