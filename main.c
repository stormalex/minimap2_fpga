...
int main(int argc, char *argv[])
{
	...
	mm_mapopt_t opt;
	mm_idxopt_t ipt;
	int i, c, n_threads = 3, sim_threads = 1, long_idx;
	char *fnw = 0, *rg = 0, *s;
	FILE *fp_help = stderr;
	...
	while ((c = getopt_long(argc, argv, opt_str, long_options, &long_idx)) >= 0) {
        ...
        else if (c == 'W') sim_threads = atoi(optarg);
...
//		fprintf(fp_help, "    -v INT       verbose level [%d]\n", mm_verbose);
	...
	snd_ctrl.sw_ring_buf = ring_alloc(SW_QUEUE_NUM);
	rcv_ctrl.stop = 0;
	rcv_ctrl.dp_ring_buf = ring_alloc(DP_QUEUE_NUM);
	...
	ring_free(snd_ctrl.sw_ring_buf);
	ring_free(x86_ctrl.sw_ring_buf);
	ring_free(rcv_ctrl.sw_ring_buf);
